...
# Version:: 0.1
# Licence:: Public domain (unlike other files)
# 
# This library is based on the code written by Chihiro Ito, Simple JSON
# parser and Simple JSON builder.
...
module AnJSON
	...
	#
	#   ruby_obj = Parser.new.parse(json_str)
	#
	...
	# *ruby_obj* like below.
	#
	#   json_str = Builder.new.build(ruby_obj)
	#
	...
	# extension to Simple JSON Builder. (like pretty print)
	class PrettyBuilder < Builder

	  # Additional Option:
		#
	  # [:indent]
	  #     indent string. default = '  ' (nbsp * 2)
	  def initialize(options = {})
			super
	    @default_indent = options.has_key?(:indent) ? options[:indent] : '  '
		end
		
	  def build(obj, options = {})
	    @indent = options.has_key?(:indent) ? options[:indent] : @default_indent
	    super
	  end
	  ...
	  def build_array(obj, level)
	    raise ERR_NestIsTooDeep if level >= @max_nest
	    "[\n" + obj.map { |item|
				(@indent * (level + 1)) + build_value(item, level)
			}.join(",\n") + "\n" + (@indent * level) + ']'
	  end
	  ...
	  def build_object(obj, level)
	    raise ERR_NestIsTooDeep if level >= @max_nest
	    "{\n" + obj.map do |item|
	      (@indent * (level + 1)) + "#{build_value(item[0].to_s,level)}: #{build_value(item[1],level)}"
	    end.join(",\n") + "\n" + (@indent * level) + "}\n"
	  end
	end
	...
	PARSER = Parser.new
	BUILDER = Builder.new
	PRETTY_BUILDER = PrettyBuilder.new

	class << self
		def parse(json, opts = {})
			PARSER.parse(json, opts)
		end
		alias load parse
		
		def build(obj, opts = {})
			BUILDER.build(obj, opts)
		end
		alias unparse   build
		alias dump      build
		alias serialize build
		
		def pretty_build(obj, opts = {})
			PRETTY_BUILDER.build(obj, opts)
		end
		alias pretty_unparse   pretty_build
		alias pretty_dump      pretty_build
		alias pretty_serialize pretty_build
	end
